...
static void interactive_power_init(struct hi6250_power_module __unused *hi6250)
{
...
static int interactive_boostpulse(struct hi6250_power_module *hi6250)
{
    char buf[80];
    int len;

   if (hi6250->boostpulse_fd < 0)
        hi6250->boostpulse_fd = open(INTERACTIVE_BOOSTPULSE_PATH, O_WRONLY);

    if (hi6250->boostpulse_fd < 0) {
        if (!hi6250->boostpulse_warned) {
            strerror_r(errno, buf, sizeof(buf));
            ALOGE("Error opening %s: %s\n", INTERACTIVE_BOOSTPULSE_PATH,
                      buf);
            hi6250->boostpulse_warned = 1;
        }
        return hi6250->boostpulse_fd;
    }

    len = write(hi6250->boostpulse_fd, "1", 1);
    if (len < 0) {
        strerror_r(errno, buf, sizeof(buf));
        ALOGE("Error writing to %s: %s\n",
...
static void
hi6250_devfreq_set_interactive(struct hi6250_power_module __unused *hi6250, int on)
{
...
static void hi6250_devfreq_init(struct hi6250_power_module __unused *hi6250)
{
...
int schedtune_sysfs_boost(struct hi6250_power_module *hi6250, char* booststr)
{
    char buf[80];
    int len;

    if (hi6250->schedtune_boost_fd < 0)
        return hi6250->schedtune_boost_fd;

    len = write(hi6250->schedtune_boost_fd, booststr, strlen(booststr));
    if (len < 0) {
        strerror_r(errno, buf, sizeof(buf));
...
{
    struct hi6250_power_module *hi6250 = (struct hi6250_power_module *)arg;

    while(1) {
        sem_wait(&hi6250->signal_lock);
        while(1) {
            long long now, sleeptime = 0;

            pthread_mutex_lock(&hi6250->lock);
            now = gettime_ns();
            if (hi6250->deboost_time > now) {
                sleeptime = hi6250->deboost_time - now;
                pthread_mutex_unlock(&hi6250->lock);
                nanosleep_ns(sleeptime);
                continue;
            }

            schedtune_sysfs_boost(hi6250, schedtune_boost_norm);
            hi6250_devfreq_set_interactive(hi6250, 0);
            hi6250->deboost_time = 0;
            pthread_mutex_unlock(&hi6250->lock);
            break;
        }
    }
...
static int schedtune_boost(struct hi6250_power_module *hi6250)
{
    long long now;
    ...
    now = gettime_ns();
    if (!hi6250->deboost_time) {
        schedtune_sysfs_boost(hi6250, schedtune_boost_interactive);
        hi6250_devfreq_set_interactive(hi6250, 1);
        sem_post(&hi6250->signal_lock);
    }
    hi6250->deboost_time = now + schedtune_boost_time_ns;

    return 0;
}

static void schedtune_power_init(struct hi6250_power_module *hi6250)
{
    char buf[50];
    pthread_t tid;

    hi6250->deboost_time = 0;
    sem_init(&hi6250->signal_lock, 0, 1);

    hi6250->schedtune_boost_fd = open(SCHEDTUNE_BOOST_PATH, O_RDWR);
    if (hi6250->schedtune_boost_fd < 0) {
        strerror_r(errno, buf, sizeof(buf));
        ALOGE("Error opening %s: %s\n", SCHEDTUNE_BOOST_PATH, buf);
        return;
    ...
    if (hi6250->schedtune_boost_fd >= 0) {
        size_t len = read(hi6250->schedtune_boost_fd, schedtune_boost_norm,
                          PROPERTY_VALUE_MAX);
	if (len <= 0)
    ...
    pthread_create(&tid, NULL, schedtune_deboost_thread, hi6250);
}

/*[generic functions]*********************************************************/

static void hi6250_cpufreq_set_interactive(struct power_module __unused *module, int on)
{
    int i;
    ...
    for (i=0; i < max_clusters; i++) {
        if ((!on || low_power_mode) && hi6250_cpufreq_clusters[i].low_power_max[0] != '\0')
		sysfs_write(hi6250_cpufreq_clusters[i].path, hi6250_cpufreq_clusters[i].low_power_max);
	else
		sysfs_write(hi6250_cpufreq_clusters[i].path, hi6250_cpufreq_clusters[i].normal_max);
    }
    sysfs_write(INTERACTIVE_IO_IS_BUSY_PATH, on ? "1" : "0");
}


static void hi6250_cpufreq_init(struct hi6250_power_module __unused *hi6250)
{
    char buf[128];
    int len, i;
    ...
    for (i=0; i < NR_CLUSTERS; i++) {
        ...
        property_get(buf, hi6250_cpufreq_clusters[i].path, "");

        if (hi6250_cpufreq_clusters[i].path[0] == '\0') {
            if (i == 0) {
                /* In case no property was set, pick cpu0's cluster */
                strncpy(hi6250_cpufreq_clusters[i].path,
        ...
        property_get(buf, hi6250_cpufreq_clusters[i].low_power_max, "");
        len = sysfs_read(hi6250_cpufreq_clusters[i].path,
                         hi6250_cpufreq_clusters[i].normal_max,
                         PROPERTY_VALUE_MAX);
        ALOGV("Cluster: %d path: %s  low: %s norm: %s\n", i,
              hi6250_cpufreq_clusters[i].path,
              hi6250_cpufreq_clusters[i].low_power_max,
              hi6250_cpufreq_clusters[i].normal_max);
    }
    max_clusters = i;
}

static void hi6250_power_init(struct power_module __unused *module)
{
    struct hi6250_power_module *hi6250 = container_of(module,
                                              struct hi6250_power_module, base);
    hi6250_cpufreq_init(hi6250);
    hi6250_devfreq_init(hi6250);
    interactive_power_init(hi6250);
    schedtune_power_init(hi6250);
}

static void hi6250_hint_interaction(struct hi6250_power_module *mod)
{
...
static void hi6250_power_hint(struct power_module *module, power_hint_t hint,
                                void *data)
{
    struct hi6250_power_module *hi6250 = container_of(module,
                                              struct hi6250_power_module, base);
    ...
    switch (hint) {
     case POWER_HINT_INTERACTION:
        hi6250_hint_interaction(hi6250);
        break;
   ...
   case POWER_HINT_VSYNC:
    ...
    case POWER_HINT_LOW_POWER:
        low_power_mode = data;
        hi6250_cpufreq_set_interactive(module, 1);
        break;
                    ...
                    hw_device_t** device)
{
    ...
    if (strcmp(name, POWER_HARDWARE_MODULE_ID) == 0) {
        struct hi6250_power_module *dev = (struct hi6250_power_module *)calloc(1,
                sizeof(struct hi6250_power_module));
        ...
        if (dev) {
            ...
            dev->base.init = hi6250_power_init;
            dev->base.powerHint = hi6250_power_hint;
            dev->base.setInteractive = hi6250_cpufreq_set_interactive;
...
struct hi6250_power_module HAL_MODULE_INFO_SYM = {
    .base = {
        .common = {
